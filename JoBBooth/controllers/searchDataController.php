...
class userController{
    ...
    public function viewAdDash(){
        if(isset($_REQUEST["refNo"])){
            unset($_SESSION['flyerDetails']);
            $refNo = $_REQUEST["refNo"];
            $rows = $this->searchM->viewAdDash($refNo);
            $flyer = $this->searchM->getFlyer($refNo);
            $alreadyApplied = $this->searchM->alreadyApplied($refNo,$_SESSION['username']);
    ...
    public function viewAdCand(){
        ...
        if($rows){
            ...
            for($x=0;$x<count($rows);$x++){
                ...
                <td><a target="_blank" href="../views/view_Ad_cand_Dash.php?ref_no='.$rows[$x]->ref_no.'&job_title='.$rows[$x]->job_title.'">View Ad</a></td>
              </tr>
                ';
            }
    ...
    public function candDashboard(){
        ...
        // if(isset($rows)){
            ...
            if($allAds){
                echo '
                    <div class="recentW">
                    <h2><i class="fas fa-ad"></i> Advertisements
                ...
                for ($x=0;$x<count($allAds);$x++){ 
                    ...
                    <form method="POST" action="../views/cand_ad_dash.php?refNo='.$allAds[$x]->ref_no.'" target="_blank">
