...
class userController{
    ...
    public function adminReport(){
        $rows = $this->searchM->adminReport();


        if($rows){
            echo '<tr>
            <th><i class="fas fa-user"></i> Reporting User</th>
            <th><i class="fas fa-user-tag"></i> Reported User</th>
            <th><i class="fas fa-flag"></i> Subject</th>
            <th><i class="fas fa-flag"></i> Description</th>
            <th><i class="fas fa-flag"></i> Reported Date</th>
            <th><i class="fas fa-flag"></i> Status</th>
            <th></th>';

            for($x=0;$x<count($rows);$x++){
                echo '<tr>
                <td>'.$rows[$x]->reporting_user_username.'</td>
                <td>'.$rows[$x]->reported_user_username.'</td>
                <td>'.$rows[$x]->subject.'</td>
                <td>'.$rows[$x]->description.'</td>
                <td>'.$rows[$x]->date_reported.'</td>
                <td>'.$rows[$x]->status.'</td>

            <td>';
            if($rows[$x]->status=="Unresolved"){
                echo '<form action="../controllers/UserController.php" method="post">
                <input type="hidden" name="type" value="adminUserUnresolved">
                <input type="hidden" name="username" value="'.$rows[$x]->id.'">
                <button type="submit" id="adminUserUnres"><i class="fas fa-times-circle"></i> RESOLVE</button>
                <style>
                #adminUserUnres{color: white;background-color: teal;border: none;width: 85%;height: 30px;font-family: body;font-weight: bold;font-size: 15px;cursor: pointer;border-radius: 10px;

                #adminUserUnres:hover{
                    color: teal;
                    background-color: white;
                }    
                ...
                echo '<form action="../controllers/UserController.php" method="post">
                <input type="hidden" name="type" value="adminUserResolved">
                <input type="hidden" name="username" value="'.$rows[$x]->id.'">
                <button type="submit" id="adminUserUnres"><i class="fas fa-check-circle"></i> UNRESOLVE</button>
                <style>
                #adminUserUnres{color: white;background-color: red;border: none;width: 85%;height: 30px;font-family: body;font-weight: bold;font-size: 15px;cursor: pointer;border-radius: 10px;
